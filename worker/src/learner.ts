import { listSessions, getMessages, getProfile } from './db';
import type { StrategyKind } from './domain';

export interface ReflectionSnap { subject: string; date: string; adjustment?: string; notes?: string; usefulStrategy?: StrategyKind; focus?: number; progress?: number; satisfaction?: number }
export interface LearnerModel {
  ...
  calibration: { recentError: number | null; improving: boolean; samples: number }; // |confidencePre - performanceActual|
...
export async function buildLearnerModel(db: D1Database, studentId: string): Promise<LearnerModel> {
  const sessions = await listSessions(db, studentId);
  ...
  const carry = lastCompleted
    ? { lastSubject: lastCompleted.subject, lastAdjustment: lastCompleted.adjustment, unmetGoals,
  ...
  const chrono = completed.slice().reverse();
  const half = Math.max(1, Math.floor(chrono.length / 2));
  const hintAvg = async (id: string) => avg((await getMessages(db, id)).filter((m) => m.role === 'assistant').map((m) => m.hintLevel ?? 0));
  const earlyAvg = round1(avg(await Promise.all(chrono.slice(0, half).map((s) => hintAvg(s.id)))));
  const recentAvg = round1(avg(await Promise.all(chrono.slice(half).map((s) => hintAvg(s.id)))));
  ...
  const strategyEfficacy = Object.entries(byStrat).map(([kind, arr]) => ({ kind: kind as StrategyKind, avgProgress: round1(avg(arr)), n: arr.length })).sort((a, b) => b.avgProgress - a.avgProgress);

  const recentProg = avg(completed.slice(0, 3).map((s) => s.progress ?? 0));
  let level: 'high' | 'medium' | 'low' = 'medium';
  let reason = 'steady — moderate scaffolding';
  if (completed.length < 2 || recentProg < 3) { level = 'high'; reason = 'early sessions / progress still low — scaffold actively'; }
  else if (completed.length >= 4 && hintTrend.decreasing && recentProg >= 4) { level = 'low'; reason = 'independence rising — fade scaffolding'; }
  ...
  const contextPatterns = Object.entries(byPlace).map(([placeCategory, arr]) => ({ placeCategory, avgProgress: round1(avg(arr)), n: arr.length })).sort((a, b) => b.avgProgress - a.avgProgress);
  ...
  return {
    sessionsCount: sessions.length, completedCount: completed.length, recentReflections, carry,
    scaffold: { level, reason }, hintTrend, strategyEfficacy, spacing: spacing.slice(0, 3),
