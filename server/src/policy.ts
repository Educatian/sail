import type { ChatMessage, PolicyDecision, StudySession } from './domain.js';
import type { LearnerModel } from './learner.js';
...
export function decidePolicy(session: StudySession, learner: LearnerModel, messages: ChatMessage[] = []): PolicyDecision {
  ...
  if (learner.scaffold.level === 'low' && goalRate >= 0.7 && !asksForAnswer) {
    return { action: 'fade', phaseTarget, intensity: 'low', reason: 'stable progress and low scaffold need', confidence: 0.72 };
  }
  if ((asksForAnswer && !correctionSaysIndependent) || highHintCount >= 2) {
    return { action: 'prompt_control', phaseTarget, intensity: 'high', reason: asksForAnswer ? 'executive help-seeking signal' : 'repeated deep hints', confidence: 0.76 };
  }
  if (noPlan) {
    return { action: 'prompt_monitoring', phaseTarget: 'forethought', intensity: 'medium', reason: 'planning trace is missing', confidence: 0.7 };
  }
  ...
  if (calibrationGap != null && calibrationGap >= 25) {
    return { action: 'prompt_monitoring', phaseTarget, intensity: 'medium', reason: 'calibration gap suggests monitoring check', confidence: 0.66 };
  }
  if (learner.scaffold.level === 'high') {
    return { action: 'prompt_monitoring', phaseTarget, intensity: 'medium', reason: learner.scaffold.reason, confidence: 0.64 };
  }
