import type { StudySession, Condition } from './domain.js';
import type { LearnerModel } from './learner.js';

/** Per-turn SRL context snapshot (Hephaestus aspect pattern), injected as JSON. */
  ...
  return JSON.stringify(
    {
      ...
      lastReflection:
        s.focus || s.notes
          ? { focus: s.focus, progress: s.progress, satisfaction: s.satisfaction, notes: s.notes }
          : null,
    },
    null,
    2,
...
const SCAFFOLD: Record<'high' | 'medium' | 'low', string> = {
  ...
  low: `\n## Scaffolding level: LOW (fade)\nThe learner is gaining independence. Fade your support: ask them to self-explain or attempt FIRST; withhold hints until they've tried; keep prompts minimal and let them lead. Avoid over-prompting.`,
};

function learnerMemoryBlock(learner?: LearnerModel): string {
  if (!learner || learner.recentReflections.length === 0) return '';
  const lines = learner.recentReflections
    .map((r) => `- ${r.subject}: progress ${r.progress ?? '?'}/5${r.usefulStrategy ? `, worked: ${r.usefulStrategy}` : ''}${r.adjustment ? `, plan: "${r.adjustment}"` : ''}`)
    .join('\n');
