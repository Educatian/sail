// SAIL domain model — ported from PracLog's Zimmerman 3-phase Log collection.
// Shared shape between server and app (kept in sync as a copy in app/src/domain.ts).

export type Phase = 'forethought' | 'performance' | 'reflection';

/** Research condition (KPIDT manipulation): metacognitive-embedded vs plain mentor. */
...
/** Pause/resume segments = real time-on-task (PracLog TimerData). */
...
export interface StudySession {
  ...
  // reflection (1-5 Likert + free text)
  focus?: Rating;
  progress?: Rating;
  satisfaction?: Rating;
  notes?: string;
  adjustment?: string; // forward plan: "next time I will..."
  usefulStrategy?: StrategyKind; // strategy attribution: which plan helped most
...
/** Optional structured comprehension check the mentor can request (DeepTutor ask_user). */
...
/** Async research metric event (DeepTutor event-bus pattern). */
